...
@section("content")
    @php
        $postCount = method_exists($posts, 'total')
            ? $posts->total()
            : $posts->count();
    @endphp
    ...
    <div class="-mt-3 mb-10 flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <p class="max-w-3xl text-[18px] font-medium leading-7 text-[#747984]">
            Manage the articles shown on the public blog.
        </p>
    ...
    @if ($posts->count() === 0)
        <div class="rounded-[22px] border border-dashed border-[#dedfe4] bg-white px-6 py-10 text-center shadow-[0_18px_55px_rgba(17,18,22,0.04)]">
            <p class="text-base font-medium text-[#747984]">No blog posts have been added yet.</p>
        </div>
    @else
        <div class="space-y-5">
                    ...
                    <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
                            ...
                            <div class="min-w-0">
                                <div class="mb-3 flex flex-wrap items-center gap-3">
                                    ...
                                    <span
                                        @class([
                                            ...
                                            'border-[#a7dcff] text-[#149cf2]' => $post->is_published,
                                            'border-[#dedfe4] text-[#747984]' => ! $post->is_published,
                                        ])
                                    >
                                        <span
                                            @class([
                                                'h-2 w-2 rounded-full',
                                                'bg-[#149cf2]' => $post->is_published,
                                                'bg-[#747984]' => ! $post->is_published,
                                            ])
                                        ></span>
                                ...
                                @if ($post->excerpt)
                                    <p class="mt-2 max-w-3xl text-[17px] font-medium leading-6 text-[#747984]">
